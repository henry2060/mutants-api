...
export class MutantsService {
    ...
    async isMutant(mutant: MutantDTO): Promise<MutantResponse> {
      ...
      if (isNotQuadraticMatrix) {
        throw new HttpException('matrix is not quadratic', HttpStatus.BAD_REQUEST)
      }
  
      if (dna.length < AppConstants.SEQUENCE_NUMBER) {
        throw new HttpException('dna sequence need to be more than 4', HttpStatus.BAD_REQUEST)
      }
      ...
      try {
        ...
        const isMutant = countMutantSequence >= AppConstants.MINIMUM_SEQUENCE_REQUIRED;
